...
FIXTURES_DIR = os.path.join(os.path.dirname(__file__), '../../samples/x-transitive-peering')
...
def test_firewall_appliance(resources):
  ...
  appliance_firewalls = [r['values'] for r in resources if r['name']
          == 'allow-appliance-ingress']
  assert len(appliance_firewalls) == 1
  assert set(appliance_firewalls[0]['source_ranges']) == set(["10.0.0.0/16", "10.200.0.0/28"])
